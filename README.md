...
└───────┬─────────────────┬──────────────────┬────────────────────┘
        ...
        ▼                 ▼                  ▼
┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐
│  学生端 Agent │  │  教师端 Agent │  │    家长端 Agent        │
│              │  │              │  │                      │
│ • 诊断 Agent  │  │ • 备课 Agent  │  │ • 报告 Agent          │
│   薄弱点分析  │  │   教案生成    │  │   汇聚学校+家庭数据    │
│              │  │              │  │   → 综合学情报告       │
│ • 出题 Agent  │  │ • 扫描 Agent  │  │                      │
│   靶向组卷    │  │   OCR识别试卷 │  └──────────────────────┘
│              │  │              │
...
┌─────────────────────────────────────────────────────────────────┐
│                      共享 Tool 层                                 │
│  ocr_tool | analysis_tool | generate_tool | publish_tool        │
│  notify_tool | speech_tool | radar_tool | db_tool               │
└─────────────────────────────────────────────────────────────────┘
                 │
                 ▼
┌─────────────────────────────────────────────────────────────────┐
│                   AI 推理层（通义千问 qwen-plus）                  │
└─────────────────────────────────────────────────────────────────┘
```
...
router.dispatch(
    user_role="teacher",
    intent="发布作业",
    context={"class_id": "C301", "exam_images": [...]}
)

# ② 扫描 Agent：OCR 考卷识别
scan_result = ocr_tool.scan(exam_images)
# 输出: [{question_id, text, student_answers, correct_answer}]

# ③ 分析 Agent：知识点错误率统计
weak_points = analysis_tool.analyze(scan_result)
# 输出: {"二次函数图像变换": 0.65, "最值求解": 0.52, "应用题": 0.48}

# ④ 出题 Agent：AI 针对性出题
questions = generate_tool.create(
    weak_points=weak_points,
    count=15,
    difficulty_ratio={"基础": 0.6, "提高": 0.3, "挑战": 0.1}
)

# ⑤ 发布 + 通知（并行 Tool 调用）
publish_tool.send(questions, target="all_students")  # 同步到学生端
notify_tool.push(homework_info, target="parents")    # 推送家长端通知
db_tool.save(homework_record)                        # 持久化作业记录
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│                   Router Agent Prompt 结构                        │
├─────────────────────────────────────────────────────────────────┤
│ 角色：你是教育平台的任务调度中心                                    │
│ 输入：用户角色={role}，操作意图={intent}，上下文={context}          │
│ 任务：识别意图 → 选择对应 Agent → 输出调用计划                      │
│ 输出格式：{agent: "xxx_agent", tools: [...], priority: 1-3}      │
│ 约束：优先保证数据闭环，不同 Agent 间状态透传                        │
└─────────────────────────────────────────────────────────────────┘
...
│ 学情注入：{student_name} 今天薄弱点：{weak_points}                  │
│ 互动规则：喵语结尾 / 撒娇鼓励 / 不说教                              │
│ 输出约束：80字以内 / 拟人化 / 结合当天学习进度给出情感反馈            │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                   备课 Agent Prompt 结构                          │
├─────────────────────────────────────────────────────────────────┤
│ 角色：经验丰富的 {subject} 学科老师                                 │
│ 班级学情注入：{weak_points_summary}（来自分析 Agent 输出）           │
│ 课题：{grade} {subject} {lesson_title}                           │
│ 教案要求：教学目标/重难点/教学步骤/课堂互动/差异化练习设计             │
│ 出题策略：重点出错误率>{threshold}%的知识点，难度分层                 │
└─────────────────────────────────────────────────────────────────┘
```
